...
"""
修改说明（2026-03-25）：
1. 在 `enc_comet()` 中增加了短序列掩码的兼容处理。
2. 原逻辑固定使用 `mask[:, :, 1]`，当某些推理样本的常识序列长度仅为 1 时会触发越界报错：
   `index 1 is out of bounds for dimension 2 with size 1`。
3. 现改为：当序列长度大于 1 时仍使用下标 1；否则回退到下标 0，
   以保证推理阶段在缺失或极短 COMET 特征下也能稳定运行。
"""
import os
import torch
...
class IAMM(nn.Module):
    ...
    def enc_comet(self, batch, i, data_type): #data_type: ctx, sit
        ...
        for r in self.rels:
            ...
            # Some inference samples may produce very short commonsense sequences
            # (e.g., only CLS after filtering). Fall back to index 0 to avoid
            # out-of-bounds when sequence length is 1.
            if mask.size(2) > 1:
    ...
    def select_topk(self, key, value, mask=None, k=config.ctx_topk): 
        ...
        available_k = key.size(-1)
        if available_k <= 0:
            raise ValueError("select_topk 收到空的 key 维度，无法执行 topk。")

        safe_k = min(k, available_k)
        scores, index = key.topk(safe_k, dim=-1, largest=True, sorted=True)

        # 推理时序列可能很短（例如只有 1-2 个候选），补齐到目标 k，
        # 保持后续 reshape 逻辑与训练时的固定 topk 维度一致。
        if safe_k < k:
            pad_count = k - safe_k
            pad_scores = scores[..., -1:].expand(*scores.shape[:-1], pad_count)
            pad_index = index[..., -1:].expand(*index.shape[:-1], pad_count)
            scores = torch.cat([scores, pad_scores], dim=-1)
            index = torch.cat([index, pad_index], dim=-1)

        index = index.view(-1, k)
            
        bz_index = torch.arange(index.shape[0], device=index.device) * value_shape[-2]
        bz_index = bz_index.unsqueeze(-1)
